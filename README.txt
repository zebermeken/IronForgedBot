...
If it is not running, confirm location of 'activate' within .venv
...
3. In your [Discord Developer Portal](https://discord.com/developers/applications), click "New Application"
    - In the Settings > OAuth2 section, use the "OAuth2 URL Generator" helper and select:
        - Scopes: `bot`, `applications.commands`
        - Bot Permissions: `Administrator`
    - Use the generated invitation URL at the bottom of the page to invite the bot to your Discord server by entering the URL into a browser
    - Additionally, reference the [Discord Developer Documentation](https://discord.com/developers/docs/intro)
4. In your Discord Developer Portal, go to the previously created application > Bot > Privileged GateWay Intents and make sure the Presence Intent, Server Members Intent, and Message Content are enabled.
	- Not doing so will lead to this error when attempting to run the bot:
...
1. If you have never used Google Cloud Platform (GCP) go to https://console.cloud.google.com/home/dashboard, sign in, and create a new project. It can be named anything. If you have a project in GCP already then move on to the next step.

2. Go to GCP, in any of your projects, create a new SA -> Manage Keys -> Add -> JSON and download it.

3. Save it in the root directory of your local IronForgedBot folder. Rename it to `service.json`.

4. Inside service.json there is a 'client_email`. Go to the spreadsheet and share it with the `client_email` from the generated key. Make sure it has edit privileges.
	-If it's a fresh GCP project, or you never enabled Google Sheets API, there will be 403 error with the direct link
	to enable it. You might need to wait for a few minutes for changes to kick in.
